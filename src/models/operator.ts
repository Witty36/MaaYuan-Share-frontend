...
export const alternativeOperatorSkillUsages: DetailedOperatorSkillUsage[] = operatorSkillUsages.map(
  (item) => ({
    ...item,
    title: item.altTitle,
  }),
...
export function findOperatorSkillUsage(
  ...
  return (
    operatorSkillUsages.filter(isChoice).find((item) => item.value === value) || unknownSkillUsage
  );
}
